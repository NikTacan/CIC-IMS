...
if (isset($_POST['report-inventory-specificRemark-allDate'])) {
    ...
    $assignedItems = $model->getInvSpecificRemarkAllDateReport($note_id);
    ...
    $logoWidth = 110; // Width of the logo
    ...
    $sheet->setCellValue('B8', 'INVENTORY REPORT');
    ...
    $sheet->getStyle('B8')->getFont()->setBold(true);
     ...
     $sheet->setCellValue('B9', '______' . $remarks . '_____');
    ...
    if (!empty($assignedItems)) {
        ...
        foreach ($assignedItems as $items) {
            ...
            $sheet->setCellValue('J' . $rowIndex, $items['note_name']);
    ...
    if (!empty($assignedItems)) {
        ...
        $sheet->setCellValue('B' . ($rowIndex + 6), '_______________'. date('M. d, Y', strtotime($date_now)) .'________________');
    ...
    $sheet->getColumnDimension('K')->setWidth($widthMd); // Acquisition Date
    ...
    $filename = 'Inventory-Report-' . $remarks  . '-' . $date_now . '.xlsx'; // Specify the file name
