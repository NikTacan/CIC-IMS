...
if (isset($_POST['assignment-excel'])) {
    ...
    $end_user = $assignment_details['username'];
    ...
    $sheet->setCellValue('A11', 'Accountable End User : ' . $end_user);
    $sheet->getStyle('A11:C11')->getFont()->setBold(true);

    $sheet->mergeCells('F11:G11');
    $sheet->setCellValue('F11', 'Assumption Date : ' . date('M. d, Y', strtotime($assignment_details['date_added'])));
    $sheet->getStyle('F11')->getFont()->setBold(true);

    $sheet->setCellValue('A12', 'PROPERTY NUMBER');
    $sheet->setCellValue('B12', 'DESCRIPTION');
    $sheet->setCellValue('C12', 'UNIT');
    $sheet->setCellValue('D12', 'QUANTITY');
    $sheet->setCellValue('E12', 'UNIT VALUE');
    $sheet->setCellValue('F12', 'TOTAL AMOUNT');
    $sheet->setCellValue('G12', 'ACQUISITION DATE');
    ...
    if (!empty($assignment_items)) {
        ...
        foreach ($assignment_items as $items) {
            ...
            $sheet->setCellValue('G' . $rowIndex, date('M. d, Y', strtotime($items['acquisition_date'])));
    ...
    if (!empty($assignment_items)) {
        ...
        $sheet->setCellValue('A' . ($rowIndex + 4), 'Signatue over Printed Name');
        ...
        $sheet->setCellValue('D' . ($rowIndex + 4), 'Signatue over Printed Name');
