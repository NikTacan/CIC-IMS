...
if (isset($_POST['report-assignment-specificDate-allUser'])) {
    ...
    $sheet->getStyle('B8')->getFont()->setBold(true);
     ...
     $sheet->setCellValue('B9', '______Overall Inventory Assignment_____');
    ...
    if (!empty($assignmentItems)) {
        ...
        foreach ($assignmentItems as $items) {
            if(!empty($items['property_no'])) {
                // Insert data into spreadsheet cells
                $sheet->setCellValue('B' . $rowIndex, $items['username']);
                $sheet->setCellValue('C' . $rowIndex, $items['description']);
                $sheet->setCellValue('D' . $rowIndex, $items['property_no']);
                $sheet->setCellValue('E' . $rowIndex, $items['qty']);
                $sheet->setCellValue('F' . $rowIndex, $items['unit']);
                $sheet->setCellValue('G' . $rowIndex, $items['unit_cost']);
                $sheet->setCellValue('H' . $rowIndex, $items['total_cost']);
                $sheet->setCellValue('I' . $rowIndex, date('M. d, Y', strtotime($items['acquisition_date'])));
                $sheet->setCellValue('J' . $rowIndex, date('M. d, Y', strtotime($items['date_added'])));
    ...
    if (!empty($assignmentItems)) {
        ...
        $sheet->setCellValue('B' . ($rowIndex + 4), 'Signatue over Printed Name');
        ...
        $sheet->setCellValue('F' . ($rowIndex + 4), 'Signatue over Printed Name');
    ...
    $widthMx = 20.00;
    $widthLg = 40.00;
    $sheet->getColumnDimension('B')->setWidth($widthMd); // Accountable End User
    $sheet->getColumnDimension('C')->setWidth($widthLg); // Description
    $sheet->getColumnDimension('D')->setWidth($widthMd); // Property Number
    $sheet->getColumnDimension('E')->setWidth($widthMd); // Quantity
    $sheet->getColumnDimension('F')->setWidth($widthMd); // Unit of Measurement
    $sheet->getColumnDimension('G')->setWidth($widthMd); // Unit Value
    $sheet->getColumnDimension('H')->setWidth($widthMd); // Amount
    $sheet->getColumnDimension('I')->setWidth($widthMx); // Property Assigned to End User
    $sheet->getColumnDimension('J')->setWidth($widthMx); // Property Assigned to End User



    $sheet->getStyle('B')->getAlignment()->setWrapText(true); // Accountable End User
    $sheet->getStyle('C')->getAlignment()->setWrapText(true); // Description
    $sheet->getStyle('D')->getAlignment()->setWrapText(true); // Property Number
    $sheet->getStyle('E')->getAlignment()->setWrapText(true); // Quantity
    $sheet->getStyle('F')->getAlignment()->setWrapText(true); // Unit Of Measurement
    $sheet->getStyle('G')->getAlignment()->setWrapText(true); // Unit Value
    $sheet->getStyle('H')->getAlignment()->setWrapText(true); // Amount
    $sheet->getStyle('I')->getAlignment()->setWrapText(true); // Property Assigned to End User
    $sheet->getStyle('J')->getAlignment()->setWrapText(true); // Property Assigned to End User
    ...
    $filename = 'Inventory-Assignment-Overall-Between-' . $fromDate . '-' . $toDate . '.xlsx'; // Specify the file name
