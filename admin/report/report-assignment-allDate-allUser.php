...
if (isset($_POST['report-excel-assignment-all-all'])) {
    ...
    $transferredItems = $model->getAllTransferredInventoryAssignment();
    ...
    if (!empty($assignmentItems || $transferredItems)) {
        ...
        foreach ($transferredItems as $transferredItem) {
			if(!empty($transferredItem['property_no'])) {
                ...
                $sheet->setCellValue('I' . $rowIndex, date('F d, Y', strtotime($transferredItem['acquisition_date'])));
                $sheet->setCellValue('J' . $rowIndex, date('F d, Y', strtotime($transferredItem['date_transferred'])));
