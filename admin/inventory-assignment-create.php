				...
				<div class="widget-box">
					...
					<div class="widget-inner">
						...
						<form action = "<?php echo $_SERVER['PHP_SELF']; ?>"  method="post" id="parAddInvForm">			
									...
									<?php if (!empty($inventories)): ?>
										<?php foreach ($inventories as $inventory): ?>
											...
											<tr>
												...
												<td>
													<?php if ($qty_max <= 0): ?>
														<span class="text-center p-1 radius-xl" style="font-size: 11px; color: white; background-color: red;">Unavailable</span>
													<?php else: ?>
							...
							/* Create inventory assignment record controller */
								...
								if (!empty($selectedItems)) {
									foreach ($selectedItems as $inv_id) {
										...
										if (!empty($result)) {
											...
											echo "<script>alert('Empty Inventory!');window.open('inventory-assignment-view.php?id=$assignment_id', '_self');</script>";
										}
									}
								} else {
									echo "<script>alert('No selected item!');window.open('inventory-assignment-view.php?id=$assignment_id', '_self');</script>";
								}
							}
