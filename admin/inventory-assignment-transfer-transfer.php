...
<?php
	...
	if (isset($_GET['new_id']) && isset($_GET['old_id'])) {
        ...
        $newAssignmentDetails = $model->getAssignmentTransferDetailById($newAssignmentId); // New Assignment Transferred Details
        $oldAssignmentDetails = $model->getAssignmentTransferDetailById($oldAssignmentId); // Old Assignment Transferred Details
		$assignment_items = $model->getTransferItemsById($oldAssignmentId);
					...
					<div class="widget-inner">
						<div class="row" width="100%">
							...
							<?php 
								...
								/* Insert transferred properties from old inventory assignemnt controller */
								if (isset($_POST['save_edit'])) {
									...
									if (!empty($selectedItems)) {
										foreach ($selectedItems as $property_no) {
											...
											$inventoryDetails = $model->getAssignmentTransferItemDetails($old_assignment_id, $property_no);
											...
											if ($inventoryDetails) {
												...
												if ($remaining_qty > 0) {
													...
													$model->updateAssignmentPropertyItemAfterTransfer($remaining_qty, $old_assignment_id, $property_no);
													...
													$model->insertTransferPropertyItem($new_assignment_id, $property_no, $description, $location, $unit, $current_qty, $unit_cost, $total_cost, $acquisition_date);
												}
												...
												echo "<script>alert('Failed to fetch inventory details for property number: $property_no');window.open('inventory-assignment-transfer-view.php?id=$new_assignment_id', '_self');</script>";
											}
										}
									} else {
								...
								/* Close transfer properties form */
